...
it("converts Markdown to HTML(GFM=false)", async () => {
    ...
    const sample = await fs.readFile("test/fixtures/sample.md", { encoding: 'utf-8'});
    const expected = await fs.readFile('test/fixtures/expected.html', { encoding: 'utf-8'});
...
it("converts Markdown to HTML (GFM=true)", async () => {
    const sample = await fs.readFile('test/fixtures/sample.md', { encoding: 'utf-8'});
    const expected = await fs.readFile('test/fixtures/expected-gfm.html', { encoding: 'utf-8'});
